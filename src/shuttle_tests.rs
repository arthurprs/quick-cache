...
use crate::{
    ...
    sync::{EntryResult, GuardResult, OccupiedAction},
};
        ...
        let task = spawn(async move {
            ...
            loop {
                ...
                let cache_fut = std::pin::pin!(cache.entry_mut_async(&0, move |_k, v| {
                    ...
                    match action {
                        0 => OccupiedAction::Keep(*v),
                        1 => OccupiedAction::Remove,
                        _ => OccupiedAction::Replace,
                    }
                }));
                let cache_fut_res = tokio::select! {
                    biased;
    ...
    thread::scope(|s| {
        ...
        loop {
            match fut.as_mut().poll(&mut cx2) {
                Poll::Ready(result) => {
                    match result {
                        EntryResult::Value(v) => assert_eq!(v, 42),
                        _ => panic!("expected EntryResult::Value"),
                    }
                    break;
                }
                Poll::Pending => {
